...
scriptPolicyName=""
until [[ -f "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1/Tokens-${scriptPolicyName}.plutus"  ]]
do
    ...
    if ! [[ -f "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1/Tokens-${scriptPolicyName}.plutus" ]]
    then
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        printf "%s\n%s\n%s\n" "17" "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1" "Tokens-${scriptPolicyName}" "$walletTxIn" "$token_name" "$token_cantidad" 

        printf "%s\n%s\n%s\n" "17" "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1" "Tokens-${scriptPolicyName}" "$walletTxIn" "$token_name" "$token_cantidad" | cabal exec deploy-auto
