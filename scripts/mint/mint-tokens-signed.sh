...
scriptPolicyName=""
until [[ -f "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1/Tokens-Signed-${scriptPolicyName}.plutus"  ]]
do
    ...
    if ! [[ -f "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1/Tokens-Signed-${scriptPolicyName}.plutus" ]]
    then
        printf "\nMinting Policiy file Tokens-Signed-${scriptPolicyName}.plutus no existe\n"
    fi
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        echo "Polcy en base de: Add: $walletAddr, TxOutRef: $walletTxIn, con el Token Name: $token_name y cantidad: $token_cantidad"
        ...
        printf "%s\n%s\n%s\n" "171" "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1" "Tokens-Signed-${scriptPolicyName}" "$walletAddr" "$walletTxIn" "$token_name" "$token_cantidad" 

        printf "%s\n%s\n%s\n" "171" "$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1" "Tokens-Signed-${scriptPolicyName}" "$walletAddr" "$walletTxIn" "$token_name" "$token_cantidad" | cabal exec deploy-auto
...
policyFile="$PLUTUS_DEVS_SCRIPTS_FILES/mintingpolicies/V1/Tokens-Signed-${scriptPolicyName}.plutus"
...
if [[ $opcion = "y" ]]; then 
    ...
    unsignedFile=$PLUTUS_DEVS_SCRIPTS_FILES/transacciones/Tokens.unsigned
    signedFile=$PLUTUS_DEVS_SCRIPTS_FILES/transacciones/Tokens.signed
